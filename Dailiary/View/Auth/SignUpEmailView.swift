...
struct SignUpEmailView: View {
    ...
    @EnvironmentObject var authViewModel: AuthViewModel // ** 서버 연동 후 필요한 코드 **
    ...
    func checkEmailDuplicated() {
        Task {
            if await authViewModel.isEmailDuplicated(currentUserEmail: email) {
                // 이메일이 중복될 경우
                isDuplicated = true
    ...
    var body: some View {
        VStack {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    HStack {
                        ...
                        .onChange(of: email) { newValue in
                            authViewModel.emailDuplicationState = .duplicated
                        }
                        ...
                        if !email.isEmpty && checkEmailRule(string: email) {
                            ...
                            if authViewModel.emailDuplicationState == .duplicated {
                                Button {
                            ...
                            } else if authViewModel.emailDuplicationState == .checking{
                                ProgressView()
            ...
            NavigationLink {
                SignUpNicknameView(email: $email, password: $password, navStack: $navStack)
            } label: {
                Text("다음")
                    .modifier(MaxWidthColoredButtonModifier(cornerRadius: 15))
            } // NavigationLink - 다음
            // TextField가 비어있거나 비밀번호 2개가 다를 경우에는 "다음" 버튼 비활성화
            .disabled(email.isEmpty || password.isEmpty || passwordCheck.isEmpty || password != passwordCheck || !checkEmailRule(string: email) || !checkPasswordRule(password: password) || authViewModel.emailDuplicationState != .notDuplicated ? true : false)
        ...
        .popup(isPresented: $isDuplicated) {
            HStack {
                Image(systemName: "exclamationmark.circle")
        ...
        .popup(isPresented: $isNotDuplicated) {
            HStack {
                Image(systemName: "checkmark.circle")
                    .foregroundColor(.white)
                Text("사용 가능한 이메일이에요.")
...
struct SignUpEmailView_Previews: PreviewProvider {
    static var previews: some View {
        SignUpEmailView(navStack: .constant(NavigationPath()))
    }
}
