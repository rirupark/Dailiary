...
struct SignUpNicknameView: View {
    ...
    private func signUpWithEmailPassword() {
        Task {
            if await authViewModel.createUser(email: email, password: password, nickname: nickName) {
    ...
    func checkNicknameDuplicated() {
        Task {
            if await authViewModel.isNicknameDuplicated(currentUserNickname: nickName) {
                // 이메일이 중복될 경우
                isDuplicated = true
    ...
    var body: some View {
        VStack {
                ...
                HStack {
                    TextField("닉네임 (20자리 이내)", text: $nickName)
                        ...
                        .onChange(of: nickName) { newValue in
                            authViewModel.nickNameDuplicationState = .duplicated
                        }
                    ...
                    if !nickName.isEmpty {
                        ...
                        if authViewModel.nickNameDuplicationState == .duplicated {
                            Button {
                        ...
                        } else if authViewModel.nickNameDuplicationState == .checking{
                            ProgressView()
            ...
            Button {
                ...
                Task {
                    if authViewModel.nickNameDuplicationState == .notDuplicated {
            ...
            .disabled(nickName.isEmpty || nickName.count > 20 || authViewModel.nickNameDuplicationState != .notDuplicated ? true : false)
        } // VStack
        .toolbar {
        ...
        .popup(isPresented: $isShowSucceedToast) {
            HStack {
                Image(systemName: "checkmark.circle")
...
struct SignUpNicknameView_Previews: PreviewProvider {
    static var previews: some View {
        SignUpNicknameView(email: .constant(""), password: .constant(""), navStack: .constant(NavigationPath()))
    }
}
